...
const COLORS = { protein: '#4ECDC4', carbs: '#FF6B35', fat: '#FFD166' };
const DEFAULT_RATIO = {
...
export default function MacroPieChart({ protein = 0, carbs = 0, fat = 0, macroRatio, isPersonalized }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 card-shadow">
      <div className="rounded-xl p-3" style={{ background: 'linear-gradient(135deg, #F0FFFE 0%, #FFFFFF 60%)' }}>
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-sm font-semibold text-text-main">三大营养素</h3>
          {isPersonalized && (
            <span className="text-[10px] bg-[#FFF8F5] text-[#FF6B35] px-1.5 py-0.5 rounded-full font-medium">个性化</span>
          )}
        </div>
        <div className="flex items-center">
          ...
          <div className="w-[45%] space-y-2.5 text-xs">
            {data.map((d) => {
              const key = d.name === '蛋白质' ? 'protein' : '碳水化合物' ? 'carbs' : 'fat';
              const rec = ratio[key];
              const pct = totalCal > 0 ? ((d.value / totalCal) * 100).toFixed(1) : 0;
              const inRange = rec && pct >= rec.min && pct <= rec.max;
              ...
              return (
                <div key={d.name} className="flex items-center gap-2">
                  ...
                  {rec && (<span className={`text-[10px] ${inRange ? 'text-green-500' : devColor}`}>{inRange ? '✓' : deviation}</span>)}
                </div>
              );
            })}
